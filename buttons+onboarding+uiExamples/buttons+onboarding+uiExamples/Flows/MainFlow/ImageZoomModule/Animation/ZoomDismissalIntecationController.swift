...
class ZoomDismissalInteractionController: NSObject {
    ...
    func didPanWith(gestureRecognizer: UIPanGestureRecognizer) {
        ...
        fromVC.navigationController?.navigationBar.isTranslucent = false
        
        transitionImageView.transform = CGAffineTransform(scaleX: scale, y: scale)
        var newCenter = CGPoint(x: anchorPoint.x, y: anchorPoint.y + translatedPoint.y - transitionImageView.frame.height * (1 - scale) / 1.2)
        
        if newCenter.y < anchorPoint.y {
            newCenter.y = anchorPoint.y
        }
